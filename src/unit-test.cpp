...
typedef testing::Types<cstlm::index_succinct<default_cst_byte_type>,
                       cstlm::index_succinct<cstlm::default_cst_int_type>>
AllImplementations;
...
TYPED_TEST(LMTest, PrecomputedStats_nX)
{
    ...
    pattern_type text;
    std::copy(
    this->idx.cst.csa.text.begin(), this->idx.cst.csa.text.end(), std::back_inserter(text));

    /* count the number of ngrams without sentinals */
    for (size_t cgram = 2; cgram <= this->idx.discounts.max_ngram_count; cgram++) {
        ...
        for (const auto& ngc : ngram_counts) {
            ...
            if (std::none_of(cng.cbegin(), cng.cend(), [](value_type i) { return i == EOS_SYM; }) &&
                std::none_of(cng.cbegin(), cng.cend(), [](value_type i) { return i == EOF_SYM; })) {
                auto cnt = ngc.second;
                switch (cnt) {
                    case 1:
                        act_n1++;
        ...
        /* compare counts */
        EXPECT_EQ(act_n1, this->idx.discounts.counts.n1[cgram]) << "n1[" << cgram
                                                                << "] count incorrect!";
        EXPECT_EQ(act_n2, this->idx.discounts.counts.n2[cgram]) << "n2[" << cgram
                                                                << "] count incorrect!";
        EXPECT_EQ(act_n3, this->idx.discounts.counts.n3[cgram]) << "n3[" << cgram
                                                                << "] count incorrect!";
        EXPECT_EQ(act_n4, this->idx.discounts.counts.n4[cgram]) << "n4[" << cgram
                                                                << "] count incorrect!";
    }
}
...
TYPED_TEST(LMTest, PrecomputedStats_nX_cnt)
{
    ...
    for (size_t cgram = 2; cgram <= this->idx.discounts.max_ngram_count; cgram++) {
        ...
        /* compare counts */
        EXPECT_EQ(this->idx.discounts.counts.n1_cnt[cgram], act_n1_cnt) << "n1_cnt[" << cgram
                                                                        << "] count incorrect!";
        EXPECT_EQ(this->idx.discounts.counts.n2_cnt[cgram], act_n2_cnt) << "n2_cnt[" << cgram
                                                                        << "] count incorrect!";
        EXPECT_EQ(this->idx.discounts.counts.n3_cnt[cgram], act_n3_cnt) << "n3_cnt[" << cgram
                                                                        << "] count incorrect!";
        EXPECT_EQ(this->idx.discounts.counts.n4_cnt[cgram], act_n4_cnt) << "n4_cnt[" << cgram
                                                                        << "] count incorrect!";
    }
}
...
TYPED_TEST(LMTest, PrecomputedStats_N1DotPlusPlus)
{
    ...
    for (size_t i = 0; i < (text.size() - 3) - 1; i++) {
        ...
        if (std::none_of(
            cur_gram.cbegin(), cur_gram.cend(), [](value_type i) { return i == EOS_SYM; }) &&
            std::none_of(
            cur_gram.cbegin(), cur_gram.cend(), [](value_type i) { return i == EOF_SYM; })) {
...
TYPED_TEST(LMTest, PrecomputedStats_N3plus_dot)
{
    ...
    for (size_t i = 0; i < (text.size() - 3); i++) {
        auto sym = text[i];
        if (sym != EOS_SYM && sym != EOF_SYM) unigram_freqs[sym]++;
    }
    size_t act_N3plus_dot = 0;
    for (const auto& uc : unigram_freqs) {
        if (uc.second >= 3) act_N3plus_dot++;
    }
    /* compare counts */
...
TYPED_TEST(LMTest, N1PlusBack)
{
    ...
    for (size_t cgram = 1; cgram <= this->idx.discounts.max_ngram_count + 5; cgram++) {
        ...
        for (const auto& ngc : ngram_counts) {
            ...
            if (std::none_of(cng.cbegin(), cng.cend(), [](value_type i) { return i == EOS_SYM; }) &&
                std::none_of(cng.cbegin(), cng.cend(), [](value_type i) { return i == EOF_SYM; }) &&
                std::none_of(cng.cbegin() + 1,
                             cng.cend() - 1,
                             [](value_type i) { return i == PAT_START_SYM; }) &&
                std::none_of(
                cng.cbegin() + 1, cng.cend() - 1, [](value_type i) { return i == PAT_END_SYM; })) {
                // (1) perform backward search on reverse csa to get the node [lb,rb]
                uint64_t lb, rb;
                auto     cnt = backward_search(
                this->idx.cst.csa, 0, this->idx.cst.csa.size() - 1, cng.begin(), cng.end(), lb, rb);
                EXPECT_TRUE(cnt > 0);
                if (cnt > 0) {
                    auto actual_count =
                    this->idx.N1PlusBack(this->idx.cst.node(lb, rb), cng.begin(), cng.end());
    ...
    for (size_t cgram = 1; cgram <= this->idx.discounts.max_ngram_count + 5; cgram++) {
        ...
        for (const auto& ngc : ngram_counts) {
            ...
            if (std::none_of(cng.cbegin(), cng.cend(), [](value_type i) { return i == EOS_SYM; }) &&
                ...
                if (cnt > 0) {
                    auto actual_count =
                    this->idx.N1PlusFrontBack(this->idx.cst.node(lb, rb), cng.begin(), cng.end());
    ...
    for (size_t cgram = 1; cgram <= this->idx.discounts.max_ngram_count + 5; cgram++) {
        ...
        for (const auto& ngc : ngram_counts) {
            ...
            if (std::none_of(cng.cbegin(), cng.cend(), [](value_type i) { return i == EOS_SYM; }) &&
                ...
                if (cnt > 0) {
                    auto actual_count =
                    this->idx.N1PlusFront(this->idx.cst.node(lb, rb), cng.begin(), cng.end());
...
TYPED_TEST(LMTest, N123PlusFront)
{
    ...
    for (size_t cgram = 1; cgram <= this->idx.discounts.max_ngram_count + 5; cgram++) {
        ...
        std::unordered_map<std::vector<value_type>, t_symbol_counts, vector_hasher<value_type>>
        ngram_counts;
        /* compute N1PlusFront c-gram stats */
        // -3 to ignore the last three symbols in the collection: UNK EOS EOF
        ...
        for (const auto& ngc : ngram_counts) {
            ...
            if (std::none_of(cng.cbegin(), cng.cend(), [](value_type i) { return i == EOS_SYM; }) &&
                ...
                if (cnt > 0) {
                    ...
                    this->idx.N123PlusFront(
                    this->idx.cst.node(lb, rb), cng.begin(), cng.end(), n1, n2, n3p);
                    n1p = this->idx.N1PlusFront(this->idx.cst.node(lb, rb), cng.begin(), cng.end());
...
TYPED_TEST(LMPPxTest, Perplexity)
{
    for (unsigned int i = 0; i < this->srilm_triplets.size(); i++) {
        ...
        double perplexity =
        sentence_perplexity_kneser_ney(this->idx, srilm.pattern, srilm.order, false);
...
TYPED_TEST(LMPPxTest, PerplexityMKN)
{
    for (unsigned int i = 0; i < this->kenlm_triplets_mkn.size(); i++) {
        ...
        double perplexity =
        sentence_perplexity_kneser_ney(this->idx, kenlm.pattern, kenlm.order, true, false);
...
TYPED_TEST(LMPPxTest, PerplexityMKN_Cache)
{
    for (unsigned int i = 0; i < this->kenlm_triplets_mkn.size(); i++) {
        auto   kenlm = this->kenlm_triplets_mkn[i];
        double perplexity =
        sentence_perplexity_kneser_ney(this->idx, kenlm.pattern, kenlm.order, true, true);
        EXPECT_NEAR(perplexity, kenlm.perplexity, 1e-2);
    }
}
