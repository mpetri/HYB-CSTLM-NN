...
const float SAMPLE_THRESHOLD = 1e-5;
...
struct embeddings {
  ...
  void store_binary(std::string file_name,
  ...
  void store_plain(std::string file_name,
                   cstlm::vocab_uncompressed<false> &vocab) {
    FILE *f = fopen(file_name.c_str(), "w");
    if (f) {
      fprintf(f, "%ld %ld\n", data.rows(), data.cols());
...
template <class t_itr> struct sentence_parser {
private:
  t_itr beg;
  t_itr end;
  t_itr cur;
  ...
  std::uniform_real_distribution<> subsampling_dist =
      std::uniform_real_distribution<>(0, 1);
  uint64_t m_cur_sentence_len;

public:
  const std::array<uint32_t, consts::MAX_SENTENCE_LEN> &cur_sentence =
      m_sentence_buf;

public:
  sentence_parser(t_itr _beg, t_itr _end, cstlm::vocab_uncompressed<false> &v,
                  float thres, float mf)
      : beg(_beg), end(_end), cur(beg), vocab(v), sample_threshold(thres),
        min_freq(mf) {}
  ...
  bool next_sentence() {
    ...
    while (cur != end) {
      ...
      if (freq < min_freq) {
        ...
        // ranking same
        if (sample_threshold) {
          float first = sqrt((float(freq) /
                              (sample_threshold * float(vocab.total_freq)))) +
                        1;
          float second =
              (sample_threshold * float(vocab.total_freq)) / float(freq);
          float prob = first * second;
          float gprob = subsampling_dist(gen);
          if (prob < gprob) {
            add_sym = false;
          }
          /*
                              double cprob = freq / double(vocab.total_freq);
                              auto   prob  = 1.0 - sqrt(sample_threshold /
          cprob);
...
struct builder {
  builder() {
    for (size_t i = 0; i < consts::EXP_TABLE_SIZE; i++) {
      ...
      m_expTable[i] =
          m_expTable[i] / (m_expTable[i] + 1); // Precompute f(x) = x / (x + 1)
    }
  }
...
private:
  ...
  float m_sample_threshold = defaults::SAMPLE_THRESHOLD;
...
private:
  ...
  std::atomic<uint64_t> m_total_sentences_skipped = ATOMIC_VAR_INIT(0);
...
private:
  void output_params() {
    ...
    cstlm::LOG(cstlm::INFO) << "W2V number of negative samples: "
                            << m_num_negative_samples;
    ...
    cstlm::LOG(cstlm::INFO) << "W2V starting learning rate: "
                            << m_start_learning_rate;
...
#pragma omp simd
  ...
  void learn_embedding_from_file_chunk(cstlm::vocab_uncompressed<false> &vocab,
    ...
    std::uniform_int_distribution<int64_t> window_dist(1, m_window_size + 1);
    std::uniform_int_distribution<int64_t> neg_sample_dist(
        1, m_unigram_bv.size() - 1);
    std::mt19937 gen(consts::RAND_SEED);
    ...
    while (sentences.next_sentence()) {
      ...
      if (sentences_processed - last_sentences_processed > 2000) {
        ...
        float cur_pos =
            sentences.cur_offset_in_text() + (cur_iteration * total_tokens);
        float text_percent = cur_pos / (float)(total_tokens * m_num_iterations);
        auto cur_time = watch::now();
        std::chrono::duration<double, std::ratio<1, 1>> secs_elapsed =
            cur_time - start;
        double sents_per_sec =
            double(sentences_processed) / (secs_elapsed.count() + 1);
        double words_per_sec =
            double(local_cur_pos) / (secs_elapsed.count() + 1);

        std::chrono::duration<double, std::ratio<1, 1>> pelapsed =
            cur_time - last_msg;
        if (pelapsed.count() > 3) {
          cstlm::LOG(cstlm::INFO)
              << "W2V [" << thread_id << "] "
              << "iter(" << cur_iteration + 1 << "/" << m_num_iterations << ") "
              << "alpha(" << std::fixed << std::setprecision(6)
              << m_cur_learning_rate << ") "
              << "S(" << std::setprecision(0) << float(sentences_processed)
              << ") "
              << "E(" << std::setprecision(0) << float(sentences_skipped)
              << ") "
              << "W(" << std::setprecision(0) << local_cur_pos << ") "
              << "S/s(" << std::setprecision(0) << sents_per_sec << ") "
              << "W/s(" << std::setprecision(0) << words_per_sec << ") "
              << "%(" << std::setprecision(2)
              << floorf(text_percent * 10000.0f) / 100.0f << ") ";
          last_msg = cur_time;
        }
      ...
      if (sentences.cur_size() <= 1) {
        sentences_skipped++;
        continue;
      }
      // we process each word in the sentence separately
      for (size_t sent_offset = 0; sent_offset < sentences.cur_size();
           sent_offset++) {
        ...
        if (start < 0)
          start = 0;
        if (stop >= (int64_t)sentences.cur_size())
          stop = sentences.cur_size() - 1;
        for (int64_t wo = start; wo < stop; wo++) {
          if (wo == (int64_t)sent_offset)
            continue; // don't examine the word itself
...
#pragma omp simd
    ...
    if (cur_iteration == 0) {
      ...
      std::atomic_fetch_add(&m_total_sentences_skipped, sentences_skipped);
    }
  }

  void learn_embedding_from_file(cstlm::vocab_uncompressed<false> &vocab,
                                 std::string file_name) {
    ...
    for (size_t j = 0; j < m_num_iterations; j++) {
      uint64_t coffset = 0;
      uint64_t chunk_size = text_size / m_num_threads;
      std::vector<std::future<void>> fchunks;
      for (size_t i = 0; i < m_num_threads; i++) {
        if (i + 1 == m_num_threads) { // last thread processes the rest
          chunk_size = text_size - coffset;
        }
        fchunks.push_back(std::async(
            std::launch::async, [&, i, j, chunk_size, coffset, file_name] {
              learn_embedding_from_file_chunk(vocab, file_name, coffset,
                                              chunk_size, i, j);
            }));
        coffset += chunk_size;
      }
    ...
    double sents_per_sec =
        double(m_total_sentences_processed) / (secs_elapsed.count() + 1);
    double words_per_sec = double(text_size) / (secs_elapsed.count() + 1);
    cstlm::LOG(cstlm::INFO)
        << "W2V Sentences Processed: " << m_total_sentences_processed << " "
  ...
  embeddings learn_embedding(cstlm::vocab_uncompressed<false> &vocab,
...
*/
    ...
    cstlm::LOG(cstlm::INFO)
        << "W2V Sentences Processed: " << m_total_sentences_processed << " "
        << "Sentences skipped: " << m_total_sentences_skipped << " "
...
public:
  ...
  embeddings train_or_load(cstlm::collection &col) {
    ...
    // (2) load vocab
    cstlm::LOG(cstlm::INFO) << "WSV create vocab";
