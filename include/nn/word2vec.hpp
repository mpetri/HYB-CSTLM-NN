...
struct embeddings {
	...
	embeddings filter(cstlm::vocab_uncompressed<false>& vocab)
	{
		embeddings filtered_emb;
		size_t	 new_vocab_size = vocab.size();
		size_t	 vector_size	= cols();
		filtered_emb.data.resize(new_vocab_size, vector_size);

		for (size_t i = 0; i < new_vocab_size; i++) {
			auto big_id = vocab.small2big(i);
			for (size_t j = 0; j < vector_size; j++) {
				filtered_emb.data(i, j) = data(big_id, j);
			}
		}
	...
	bool next_sentence()
	{
		...
		while (cur != end) {
			...
			if (freq < min_freq) {
				...
				if (sample_threshold) {
					float first =
					sqrt((float(freq) / (sample_threshold * float(vocab.total_freq)))) + 1;
					float second = (sample_threshold * float(vocab.total_freq)) / float(freq);
