...
public:
    ...
    template <class t_cst, class t_node_type>
    uint32_t compute_contexts_mkn(
    t_cst& cst, t_node_type node, uint64_t& num_syms, uint64_t& f1prime, uint64_t& f2prime)
    {
    ...
    void initialise_kneser_ney(collection& col, t_cst& cst, uint64_t max_node_depth)
    {
        ...
        auto tmp_buffer_counts_fb =
        sdsl::int_vector_buffer<32>(col.temp_file("counts_fb"), std::ios::out);
        auto tmp_buffer_counts_b =
        sdsl::int_vector_buffer<32>(col.temp_file("counts_fb"), std::ios::out);
        uint64_t num_syms = 0;
        ...
        LOG(INFO) << "precomputed " << m_bv_rank(m_bv.size()) << " entries out of " << m_bv.size()
                  << " nodes";
    }

    template <class t_cst, class t_node, class t_writer>
    void precompute_subtree(t_cst&    cst,
                            t_node    start,
                            t_writer& writer,
                            std::vector<std::pair<uint64_t, uint64_t>>& child_hist,
                            uint64_t max_depth)
    {
    ...
    void initialise_modified_kneser_ney(collection& col, t_cst& cst, uint64_t max_node_depth)
    {
        ...
        for (auto i = 0; i < num_threads; i++) {
            ...
            std::launch::async,
            [this, i, max_node_depth, &cst, &col](
            std::vector<typename t_cst::node_type> nodes) -> counts_writer {
    ...
    template <class t_cst, class t_node_type>
    void lookup_f12prime(t_cst& cst, t_node_type node, uint64_t& f1prime, uint64_t& f2prime) const
    {
#ifdef ENABLE_CSTLM_TIMINGS
