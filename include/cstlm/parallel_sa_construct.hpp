...
{
    return f(a, b) ? (f(b, c) ? b : (f(a, c) ? c : a)) : (f(a, c) ? a : (f(b, c) ? c : b));
}
...
struct bucket_counts {
    ...
    t_cnts                total_cnts;
    bucket_counts(size_t threads) { partial_cnts.resize(threads); }
};
...
bucket_counts<suftype_counts_array_byte> compute_bucket_counts(t_vec& T, size_t n)
{
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        if (i + 1 == num_threads) m = (n - 1) - start;
...
bucket_counts<suftype_counts_array_int> compute_bucket_counts(t_vec& T, size_t n)
{
    ...
    for (size_t i = 0; i < num_threads; i++) {
        auto m                      = syms_per_thread;
        if (i + 1 == num_threads) m = (n - 1) - start;
        cilk_spawn compute_partial_cnts<t_vec, suftype_counts_array_int>(
        T, start, m, b.partial_cnts[i]);
        start += m;
    }
    cilk_sync;
...
void put_B_types_in_bucket(t_vec& T, t_int_type* SA, t_cnts& b, size_t n)
{
    ...
    for (size_t i = 0; i < b.partial_cnts.size(); i++) {
        ...
        if (i + 1 == b.partial_cnts.size()) m = (n - 2) - start;
        cilk_spawn partial_put_B_types_in_bucket<t_vec, t_int_type, decltype(b.partial_cnts[i])>(
        T, start, m, SA, b.partial_cnts[i]);
        start += m;
    }
    cilk_sync;
}
...
struct sa_cmp_func_byte {
    const uint8_t* T;
    sa_cmp_func_byte(const uint8_t* _T) : T(_T) {}
    bool operator()(const t_int_type a, const t_int_type b) const
    {
        ...
        while (true) {
            ...
            int     index =
            _mm_cmpistri(x, y, _SIDD_UBYTE_OPS | _SIDD_CMP_EQUAL_EACH | _SIDD_NEGATIVE_POLARITY);
            if (index != 16) {
