...
template <class t_cst,
          ...
          uint32_t t_mgram_cache     = 3,
          uint32_t t_cache_entries   = 2000000>
...
public:
    ...
    index_succinct<t_cst, t_max_ngram_count>&
    operator=(index_succinct<t_cst, t_max_ngram_count>&& idx)
    {
        m_vocab             = std::move(idx.m_vocab);
        m_cst               = std::move(idx.m_cst);
        m_discounts         = std::move(idx.m_discounts);
    ...
    {
	{
        ...
        if (!utils::file_exists(cst_file)) {
            ...
            cfg.file_map[sdsl::conf::KEY_TEXT_INT] = col.file_map[KEY_CSTLM_TEXT];
            cfg.file_map[sdsl::conf::KEY_TEXT]     = col.file_map[KEY_CSTLM_TEXT];
            construct(m_cst, col.file_map[KEY_CSTLM_TEXT], cfg, 0);
            sdsl::store_to_file(m_cst, cst_file);
        } else {
            sdsl::load_from_file(m_cst, cst_file);
        }
        auto discounts_file =
        col.path + "/tmp/DISCOUNTS-MAXN=" + std::to_string(t_max_ngram_count) + "-BYTE=" +
        std::to_string(byte_alphabet) + "-" + sdsl::util::class_to_hash(m_discounts) + ".sdsl";
        ...
        auto precomputed_file =
        col.path + "/tmp/PRECOMPUTED_COUNTS-MAXN=" + std::to_string(t_max_ngram_count) + "-BYTE=" +
        std::to_string(byte_alphabet) + "-" + sdsl::util::class_to_hash(m_precomputed) + ".sdsl";
        ...
        auto probcache_file = col.path + "/tmp/PROBCACHE-MAXM=" + std::to_string(t_mgram_cache) +
                              "-MAXC=" + std::to_string(t_max_ngram_count) + "-BYTE=" +
    ...
    uint64_t
    N1PlusBack(const node_type& node, pattern_iterator pattern_begin, pattern_iterator) const
    {
        ...
        if (m_cst.is_leaf(node)) {
            ...
            static thread_local std::vector<typename t_cst::csa_type::wavelet_tree_type::size_type>
            ...
            m_cst.csa.wavelet_tree, lb, rb + 1, num_syms, preceding_syms, left, right);
    ...
    void print_params(bool ismkn, uint32_t ngramsize) const { m_discounts.print(ismkn, ngramsize); }
    ...
    void N123PlusFrontPrime(const node_type& node,
                            pattern_iterator pattern_begin,
                            pattern_iterator pattern_end,
                            uint64_t&        f1prime,
                            uint64_t&        f2prime,
                            uint64_t&        f3pprime) const
    {
#ifdef ENABLE_CSTLM_TIMINGS
        ...
        if (full_match) {
            if (m_precomputed.is_precomputed(m_cst, node)) {
                m_precomputed.lookup_f12prime(
                m_cst, node, f1prime, f2prime); // FIXME change the name n1plusfrontback
                all = m_cst.degree(node);
            } else {
                for (auto child = m_cst.select_child(node, 1); child != m_cst.root();
                     child      = m_cst.sibling(child)) {
                    auto lb = m_cst.lb(child);
                    auto rb = m_cst.rb(child);

                    static thread_local std::vector<
                    typename t_cst::csa_type::wavelet_tree_type::value_type>
                    preceding_syms(m_cst.csa.sigma);
                    static thread_local std::vector<
                    typename t_cst::csa_type::wavelet_tree_type::size_type>
                    left(m_cst.csa.sigma);
                    static thread_local std::vector<
                    typename t_cst::csa_type::wavelet_tree_type::size_type>
                                                        right(m_cst.csa.sigma);
                    ...
                    sdsl::interval_symbols(
                    m_cst.csa.wavelet_tree, lb, rb + 1, num_syms, preceding_syms, left, right);
                    if (num_syms == 1) f1prime++;
                    if (num_syms == 2) f2prime++;
                    all++;
                    child = m_cst.sibling(child);
                }
            }
            ...
            if (num_symsprime == 1) f1prime++;
            if (num_symsprime == 2) f2prime++;
    ...
    uint64_t compute_contexts(const t_cst&     cst,
