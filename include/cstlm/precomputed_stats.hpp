...
private:
    ...
    template <typename t_cst>
    void process_subtree(const t_cst& cst, sdsl::int_vector_buffer<>& SA, t_rank_bv& sentinel_rank, t_select_bv& sentinel_select, typename t_cst::node_type cur_node, uint64_t counter,
        raw_counts& counts, uint64_t _max_ngram_count);
