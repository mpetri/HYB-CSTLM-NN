...
\textsc{N1Plus} and \textsc{N1PlusFrontBack} can compute the requisite occurence counts for \ngram language modelling, however at considerable cost in terms of space and time. 
  ...
  \begin{algorithmic}[1]
    \Function{ProbKneserNey}{$\tf, \tr, \ws, n$} 
        ...
        \For{$i \gets 1 \text{ to } $}
          ...
          \EndIf
          \EndIf
          \Let{$D$}{lookup discount parameter for \ngram}
          \If{$i = m$} % or if s == '<s>' 
...
\textsc{N1PlusBack1} is computed in a similar way, using the \emph{interval-symbols} method to compute the number of unique preceeding symbols (see \supp, Algorithm~6).
Overall the time complexity of inference for both \textsc{N1PlusBack1} and \textsc{N1PlusFrontBack1} is $\Order{k \log \sigma}$ where $k$ is the number of preceeding symbols, a considerable improvement over \textsc{N1PlusFrontBack} using the forward and reverse \CSTs.
