...
operation after which we can compute $\nlplus{\patdot
  s}$ \ehsan{this is confusing}.\footnote{Backward search in the reverse tree corresponds to
...
Algorithm~\ref{alg-kn-slow} \ehsan{the match ... in linee 1,2,3 is a bit confusing. These nodes are not keeping track of the same thing.} presents an outline of the approach.
...
The precomputation algorithm is provided in Algorithm~6 in the \supp,
and operates by traversing the nodes of the reverse \CST and at each
stage computing the number of \ngrams that occur 1--4 times \ehsan{used for computing $D_m$ in eq.~\ref{eq:high}}, or with
$\nlplus{\dotpat} \in [1,4]$ \ehsan{used for computing $D_k$ in eq.~\ref{eq:mid}}, for various lengths of \ngrams.
...
Using the same process as backward search, described in Section~\ref{sec-css}, these ranges can be used to find the $\SA[l,r]$ interval for the extended pattern (lines 7 and 8), which uniquely determines the corresponding suffix tree node.
