...
\begin{algorithm}[htpb]
  ...
  \begin{algorithmic}[1]
    ...
    \Function{N1P}{$t, n, \alpha$} %\Comment{ $t$ is either forward or reverse \CST}
...
\begin{algorithm*}[htpb]
  \caption{Compute Kneser-Ney probability, $P\big(w_k | w^{k-1}_{k-(m-1)}\big)$, using a single \CST
    \label{alg:pkn_fwd}}
  \begin{algorithmic}[1]
    \Function{ProbKneserNey1}{$\tf, \ws, m$} 
        ...
        \For{$i \gets 1 \text{ to } m$}
          ...
          \EndIf
          \Let{$D_i$}{discount parameter for $i$gram}
          \If{$i = m$} % or if s == '<s>' 
          ...
          \Else
             ...
             \Let{$d$}{$\nlplusfrontbacklfunc{\tf}{\nf}{\Bigcdot w^{k-1}_{k-i+1} \Bigcdot}$} 
           \EndIf
           \If{$i > 1$}
...
\begin{algorithm*}[htpb]
  ...
  \begin{algorithmic}[1]
    \Function{PrecomputeDiscounts}{$\tr, m$}
        \Let{$c_{k,f}$}{$0 \quad \forall k \in [1, m] , f \in [1, 2]$} \Comment{number of $k$-grams with count $f$}
        \Let{$N^1_{k,g}$}{$0 \quad \forall k \in [1, m] , g \in [1, 2]$} \Comment{number of $k$-grams with  $N^{1+}({\dotpat}) = g$}
        ...
        \For{$\nr \gets \text{descendents}(\rooot{\tr})$} \Comment{depth-first search over nodes in \CST}
           ...
           %\If{$\leaf{\tr}{\nr}$} 
            ...
            %\Else
             ...
             %\EndIf
             \For{$k \gets d_P+1 \text{ to } \min\left(d, d_P+m\right)$}
                ...
                \Else
                     ...
                     %\State $g \gets \nlplusfunc{\tr}{\nr}{\Bigcdot \alpha}$ \Comment{retrieve occurrence count}
...
Portuguese&PT&187.20&49.03&183633&1737\\ \midrule
Wikipedia && 8637 & 9057 & 196 & 87835 \\ 
\end{tabular}}
\caption{Dataset statistics, showing total uncompressed size; and tokens, types and sentence counts for the training partition. }\label{fig:data}
\end{table}
